...
export type parseResult =
  | { ok: true; format: "json-array" | "ndjson"; logs: RawLog[] }
  | { ok: false; message: string; errors?: { line: number; errors: string }[] };
    ...
    try {
      ...
      for (let i = 0; i < parsed.length; i++) {
        ...
        if (!isObject(item)) {
          return {
            ok: false,
            message: `Expected each item in the array to be an object`,
            errors: [{ line: i + 1, errors: "Not an object" }],
          };
        }
        logs.push(item);
      }
      ...
      return {
        ...
        errors: [{ line: 0, errors: (e as Error).message }],
      };
    }
  }
